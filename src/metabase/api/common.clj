...
(s/defn reconcile-position-for-collection!
  "Compare `old-position` and `new-position` to determine what needs to be updated based on the position change"
  [model
...
(s/defn maybe-reconcile-collection-position!
  ...
  [model
   {old-collection-id :collection_id, old-position :collection_position, :as before-update} :- ModelWithPosition
